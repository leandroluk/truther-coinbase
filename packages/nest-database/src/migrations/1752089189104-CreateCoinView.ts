  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      SELECT
        ...
        cp_last."marketCap" AS "currentMarketCap",
      ...
      FROM "Coin" c
      LEFT JOIN LATERAL (
        SELECT "value", "marketCap", "updatedAt"
        FROM "CoinPrice"
        WHERE "coinId" = c."id"
      ...
      LEFT JOIN (
        ...
        FROM "CoinPrice"
        GROUP BY "coinId"
      ) AS cp_max ON cp_max."coinId" = c."id"
      LEFT JOIN (
        SELECT "coinId", MIN("value") AS "min_price"
        FROM "CoinPrice"
        GROUP BY "coinId"
      ) AS cp_min ON cp_min."coinId" = c."id"
      LEFT JOIN LATERAL (
        SELECT 
          CASE 
